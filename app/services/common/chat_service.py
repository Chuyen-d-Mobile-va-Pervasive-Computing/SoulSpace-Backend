...
class ChatService:
    ...
    async def get_conversations(self, current_user: dict) -> List[dict]:
        ...
        for chat in chats:
            if current_user["role"] == "user":
                partner_profile = await self.expert_repo.get_by_id(str(chat.expert_profile_id))
                if not partner_profile:
                    continue
                partner_user = await self.user_repo.get_by_id(str(partner_profile.user_id))
                partner_id = str(chat.expert_profile_id)  # expert_profile_id
                partner_name = partner_profile.full_name
                partner_avatar = partner_profile.avatar_url or ""
                partner_online = manager.is_online(str(chat.user_id)) if partner_user else False
                last_seen = partner_user.last_seen_at.isoformat() if partner_user and not partner_online and partner_user.last_seen_at else None
                unread = chat.user_unread
            else:
                partner_user = await self.user_repo.get_by_id(str(chat.user_id))
                if not partner_user:
                    continue
                ...
                partner_online = manager.is_online(partner_id)
    ...
    async def get_messages(self, chat_id: str, current_user: dict) -> List[dict]:
        ...
        await self.chat_repo.mark_as_read(chat_id, reader_role)
