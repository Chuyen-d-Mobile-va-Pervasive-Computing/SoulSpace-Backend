...
class JournalTreeOrchestrator:
    ...
    async def create_journal_with_enhancements(
        ...
        Trả về UTC timestamp - FE sẽ convert sang giờ VN khi hiển thị.
        """
        # 1. TẠO JOURNAL (lưu UTC vào DB)
        ...
        # 6. TRẢ VỀ RESPONSE - Trả về UTC timestamp
        # created_at trong journal là UTC naive, thêm timezone info
        utc_created = journal.created_at.replace(tzinfo=timezone.utc)
       
        return {
            "id": str(journal.id),
            "created_at": utc_created.isoformat(),  # ISO string với UTC timezone
